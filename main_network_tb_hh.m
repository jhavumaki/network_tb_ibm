...
function main_network_tb_hh(ARRAYID)
...
r=r(1:(end-9),:);
%now we sum by row for latent (column 1 and 2 correspond to state 0 and 1 and we divide by number in each community contact group and take the mean
...
%community/HH contact prevalence among detected
hhTxIDs=ceil([TxIDs]/5); %hh_ids of tx
hhContacts=[(hhTxIDs*5-4)+0 (hhTxIDs*5-4)+1 (hhTxIDs*5-4)+2 (hhTxIDs*5-4)+3 (hhTxIDs*5-4)+4]; %all members of house
hhContacts_states = reshape(state_df(hhContacts,2),size(hhContacts)); %states for all HH members
   
%intervention   
L_states =find(hhContacts_states==1 | hhContacts_states==0);
      state_df(hhContacts(L_states),2) =  8; %current state is ptL
      state_df(hhContacts(L_states),3) =  6; %next state is 6 i.e., R
      state_df(hhContacts(L_states),4) = t+6;
      
      C_states =find(hhContacts_states==2);
      state_df(hhContacts(C_states),2) =  3; %tx
      state_df(hhContacts(C_states),3) =  6; %next state is 6 i.e., R
      state_df(hhContacts(C_states),4) = t+6;

%store TPT and Tx      
newTxTemp=length(TxIDs)+length(C_states);      
newTx= newTxTemp+newTx;
newTPTTemp=  length(L_states);
newTPT= newTPT + newTPTTemp;
   
%keep track of who moved to I and who moved out
...
%adjust ATB_ID based on intervention
indx=ismember(ATB_ID(:,1),state_df(hhContacts(C_states),1)); %which people in ATB_ID were moved to Tx?
indx=find(indx); %by index
if (size(ATB_ID,1)>0)
indx=indx(ATB_ID(indx,4)>t); %which ones are CURRENTLY in ATB_ID? not greater than or equals just greater than
ATB_ID(indx,3)=3; %change their state to Tx
ATB_ID(indx,4)=t;%change the time to now
end
...
filename_ts= strcat('state_ts','net', num2str(network),'array',num2str(array), 'lhs', num2str(l),'rng',num2str(rdmseeds(l,ss)),'seed', num2str(ss),'hh','.csv')
...
filename_ts= strcat('ATB_comm_hh','net', num2str(network),'array',num2str(array), 'lhs', num2str(l),'rng',num2str(rdmseeds(l,ss)),'seed', num2str(ss),'hh','.csv');
fid = fopen(filename_ts, 'w') ;
fclose(fid);
dlmwrite(filename_ts, HH_Comm_ATB,'roffset',1, '-append')

%save length of ATB time; infectious period
filename_ts= strcat('ATB_infPer','net', num2str(network),'array',num2str(array), 'lhs', num2str(l),'rng',num2str(rdmseeds(l,ss)),'seed', num2str(ss),'hh','.csv');
fid = fopen(filename_ts, 'w') ;
fclose(fid);
...
newTxAll(ppp)=newTx;
newTPTAll(ppp)=newTPT;
id_name{ppp}=strcat('lhs',num2str(l),'seed', num2str(rdmseeds(l,ss)), 'hh');
ppp= ppp+1;
...
%save other files 1 per cluster job 
 ...
 for ss= 1:totalrandseed
    ...
    iMatNames{k} = char(strcat('lhs',num2str((l)),'seed', num2str(rdmseeds(l,ss)), 'hh'));
...
%annual incidence
  header= iMatNames;
 filename_dfo= strcat('inc_', 'lhs',num2str(minLHS), 'hh',date, '.csv'); 
...
%community attributable infections
  header= iMatNames;
 filename_dfo= strcat('annualC_', 'lhs',num2str(minLHS), 'hh',date, '.csv'); 
 fid = fopen(filename_dfo, 'w') ;
 fprintf(fid, '%s,', header{:}) ;
 fclose(fid);
...
%househod attributable infections
 header= iMatNames;
 filename_dfo= strcat('annualHH_', 'lhs',num2str(minLHS), 'hh',date, '.csv'); 
 fid = fopen(filename_dfo, 'w') ;
 fprintf(fid, '%s,', header{:}) ;
 fclose(fid);
...
%new treatment and TPT

TxTPT_table=table([newTxAll]', [newTPTAll]', id_name');
TxTPT_table.Properties.VariableNames={'newTx' 'newTPT', 'file'} ;
 filename_dfo= strcat('TxTPT_', 'lhs',num2str(minLHS), 'hh',date, '.csv'); 
writetable( TxTPT_table, filename_dfo, 'writevariablenames', true, 'quotestrings', true,'Delimiter', ",")
