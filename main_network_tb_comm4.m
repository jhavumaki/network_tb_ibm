...
%intervention
Community= datasample(IDs,length(TxIDs)*4,'Replace', false); %sample with replacement from all ids including treated IDs, more conservative
      Comm_states = reshape(state_df(Community,2),size(Community)); %states for all HH members
   
L_states =find(Comm_states ==1 | Comm_states ==0);
      state_df(Community(L_states),2) =  8; %current state is ptL
      state_df(Community(L_states),3) =  6; %next state is 6 i.e., R
      state_df(Community(L_states),4) = t+6;
      
      C_states =find(Comm_states ==2);
      state_df(Community(C_states),2) =  3; %tx
      state_df(Community(C_states),3) =  6; %next state is 6 i.e., R
      state_df(Community(C_states),4) = t+6;

%store TPT and Tx      
newTxTemp=length(TxIDs)+length(C_states);      
newTx= newTxTemp+newTx;
newTPTTemp=  sum(sum(Comm_states ==1 | Comm_states ==0));
...
%adjust ATB_ID i.e., mnext state and next time among infectious people based on intervention
indx=ismember(ATB_ID(:,1),state_df(Community(C_states),1)); %which people in ATB_ID were moved to Tx?
...
filename_ts= strcat('state_ts','net', num2str(network),'array',num2str(array), 'lhs', num2str(l),'rng',num2str(rdmseeds(l,ss)),'seed', num2str(ss),'comm4','.csv')
...
filename_ts= strcat('ATB_comm_hh','net', num2str(network),'array',num2str(array), 'lhs', num2str(l),'rng',num2str(rdmseeds(l,ss)),'seed', num2str(ss),'comm4','.csv');
fid = fopen(filename_ts, 'w') ;
fclose(fid);
dlmwrite(filename_ts, HH_Comm_ATB,'roffset',1, '-append')

%save length of ATB time; infectious period
filename_ts= strcat('ATB_infPer','net', num2str(network),'array',num2str(array), 'lhs', num2str(l),'rng',num2str(rdmseeds(l,ss)),'seed', num2str(ss),'comm4','.csv');
fid = fopen(filename_ts, 'w') ;
fclose(fid);
...
id_name{ppp}=strcat('lhs',num2str(l),'seed', num2str(rdmseeds(l,ss)), 'comm4');
ppp= ppp+1;
...
%save other files 1 per cluster job 
 ...
 for ss= 1:totalrandseed
    ...
    iMatNames{k} = char(strcat('lhs',num2str((l)),'seed', num2str(rdmseeds(l,ss)), 'comm4'));
...
%annual incidence
  header= iMatNames;
 filename_dfo= strcat('inc_', 'lhs',num2str(minLHS), 'comm4',date, '.csv'); 
...
%community attributable infections
  header= iMatNames;
 filename_dfo= strcat('annualC_', 'lhs',num2str(minLHS), 'comm4',date, '.csv'); 
 fid = fopen(filename_dfo, 'w') ;
 fprintf(fid, '%s,', header{:}) ;
 fclose(fid);
...
%househod attributable infections
 header= iMatNames;
 filename_dfo= strcat('annualHH_', 'lhs',num2str(minLHS), 'comm4',date, '.csv'); 
 fid = fopen(filename_dfo, 'w') ;
 fprintf(fid, '%s,', header{:}) ;
 fclose(fid);
...
TxTPT_table.Properties.VariableNames={'newTx' 'newTPT', 'file'} ;
 filename_dfo= strcat('TxTPT_', 'lhs',num2str(minLHS), 'comm4',date, '.csv'); 
