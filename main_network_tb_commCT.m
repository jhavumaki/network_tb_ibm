...
function main_network_tb_commCT(ARRAYID)
...
%intervention
ScreenStates=arrayfun(@(x) community_contact_screening(x, state_df, t, CommConnects), TxIDs, 'UniformOutput', false);
       newStates=cell2mat(ScreenStates);
     

if ~isempty(newStates)  
C_states=newStates(find(newStates(:,6)==1),1);
L_states=newStates(find(newStates(:,5)==1),1);
state_df(newStates(:,1),2:4)= newStates(:,2:4);
...
indx=ismember(ATB_ID(:,1),state_df(C_states,1)); %which people in ATB_ID were moved to Tx?
...
filename_ts= strcat('state_ts','net', num2str(network),'array',num2str(array), 'lhs', num2str(l),'rng',num2str(rdmseeds(l,ss)),'seed', num2str(ss),'commCT','.csv')
...
filename_ts= strcat('ATB_comm_hh','net', num2str(network),'array',num2str(array), 'lhs', num2str(l),'rng',num2str(rdmseeds(l,ss)),'seed', num2str(ss),'commCT','.csv');
fid = fopen(filename_ts, 'w') ;
fclose(fid);
dlmwrite(filename_ts, HH_Comm_ATB,'roffset',1, '-append')

%save length of ATB time; infectious period
filename_ts= strcat('ATB_infPer','net', num2str(network),'array',num2str(array), 'lhs', num2str(l),'rng',num2str(rdmseeds(l,ss)),'seed', num2str(ss),'commCT','.csv');
fid = fopen(filename_ts, 'w') ;
fclose(fid);
...
id_name{ppp}=strcat('lhs',num2str(l),'seed', num2str(rdmseeds(l,ss)), 'commCT');
ppp= ppp+1;
...
%save other files 1 per cluster job 
 ...
 for ss= 1:totalrandseed
    ...
    iMatNames{k} = char(strcat('lhs',num2str((l)),'seed', num2str(rdmseeds(l,ss)), 'commCT'));
...
%annual incidence
  header= iMatNames;
 filename_dfo= strcat('inc_', 'lhs',num2str(minLHS), 'commCT',date, '.csv'); 
...
%community attributable infections
  header= iMatNames;
 filename_dfo= strcat('annualC_', 'lhs',num2str(minLHS), 'commCT',date, '.csv'); 
 fid = fopen(filename_dfo, 'w') ;
 fprintf(fid, '%s,', header{:}) ;
 fclose(fid);
...
%househod attributable infections
 header= iMatNames;
 filename_dfo= strcat('annualHH_', 'lhs',num2str(minLHS), 'commCT',date, '.csv'); 
 fid = fopen(filename_dfo, 'w') ;
 fprintf(fid, '%s,', header{:}) ;
 fclose(fid);
...
TxTPT_table.Properties.VariableNames={'newTx' 'newTPT', 'file'} ;
 filename_dfo= strcat('TxTPT_', 'lhs',num2str(minLHS), 'commCT',date, '.csv'); 
